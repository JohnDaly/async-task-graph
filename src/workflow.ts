...
export const makeWorkflowBuilder = <W extends UnknownWorkflowDefinition>() => {
  ...
  return {
    ...
    buildConcurrentWorkflow: (
      options: { selectedTasks?: TaskId<W>[] } = {},
    ) => {
      const graph = validateAndBuildGraph()
      ...
      const runWorkflow = async (context: W[`context`]) => {
        // Before we start, set up listeners for tasks, to track dependencies
        const listenForTaskResult = (id: TaskId<W>) => {
          return new Promise((resolve) => {
            emitter.on(`taskFinish`, ({ id: finishedTaskId, result }) => {
              if (finishedTaskId === id) resolve(result)
            })
          })
        }

        // Keep track of the pending task results
        const taskResultPromiseMap = new Map<
          TaskId<W>,
          Promise<TaskFinishArgs<W>[`result`]>
        >()

        // Set up the listeners in the map, so that we can await them later on
        for (const taskId of taskOrder) {
          taskResultPromiseMap.set(
            taskId,
            listenForTaskResult(taskId) as Promise<any>,
          )
        }

        const getTaskResult = async (id: TaskId<W>) => {
          return await taskResultPromiseMap.get(id)
        }
...
> {
  getTaskResult: <D extends DepId>(id: D) => Promise<WD[`returns`][D]>
  context: WD[`context`]
}
